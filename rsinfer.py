...
class RealSense:
    ...
    def __init__(self, width, height, fps):
        ...
        self.config.enable_stream(prs2.stream.depth, self.width, self.height, prs2.format.z16, self.fps)
        self.config.enable_stream(prs2.stream.color, self.width, self.height, prs2.format.rgba8, self.fps)
    ...
    def Start(self):
        try:
            ...
            self.running = True # Set fun flag to true
    ...
    def Capture(self):
        ...
        try:
            ...
            color_frame = self.frames.get_color_frame() # pull color frame from pipeline
            self.color_np = np.asanyarray(color_frame.get_data()) # convert frame to array
    ...
    def Align(self):
        if self.frames is None: self.Capture() # grab an image if we don't have information already
...
class RealDetector:
    ...
    def __init__(self, network, threshold=0.4):
        ...
        ## Jetson config
        self.network = ji.detectNet(network, threshold) # load the object detection network
        
        ## Create detection class dictionaries
        self.detect_dict = {clss: self.network.GetClassDesc(clss) for clss in range(self.network.GetNumClasses())}
    ...
    def Detect(self, color_np):
        self.height, self.width, self.channels = color_np.shape # extract dimensions from input array
        cuda_image = ju.cudaFromNumpy(color_np) # create cuda memory capsule from np array
        detectnet_output = self.network.Detect(cuda_image, self.width, self.height) # detect objects in the image

        if len(detectnet_output) == 0: return None # if no detections are found output nothing
...
class RealSegmenter:
    ...
    def __init__(self, network):
        ...
        self.filter_mode = "point" # ensures segmentation masks are exact, not fuzzy (linear)
        ...
        color_file = 'colors.txt' # color filename
        ## create strings out of files
        classes = [clss.strip() for clss in open(os.path.join(class_path, network, class_file))] # get list of class strings
        ...
        for color in open(os.path.join(class_path, network, color_file)):
            px = color.strip().split() # strip outside whitespace and split by space delimiters
            colors.append(tuple(map(int, px))) # add pixel values to list
    ...
    def Segment(self, color_np):
        self.height, self.width, self.channels = color_np.shape # get size of image from input
        # Allocate CUDA memory
        self.cuda_out = ju.cudaAllocMapped(self.width * self.height * 4 * ctypes.sizeof(ctypes.c_float))
    ...
    def Depth(self, rs, seg_np_out):
        if not rs.aligned: rs.Align() # align camera if not done already

        ## Find mean/median of target areas
        net_mask = np.all(seg_np_out.astype('uint8') != self.segment_dict[self.seg_class], axis=-1) # retrieve class location
        depth_array = np.ma.array(self.depth_np, mask=net_mask) # create masked depth array
        self.depth_value = float(depth_array.mean() * self.depth_scale) # get values inside mask and then scale
    ...
    def Classify(self, seg_np_out):
        found_colors = np.unique(seg_np_out.reshape(-1, seg_np_out.shape[2]), axis=0).astype(int) # find unique pixel values in image
    ...
    def Locate(self, seg_np, target_class=None):
        ...
        class_px = np.where(seg_np.astype('uint8')[:,:,:3] == np.asarray(self.reverse_dict[target_class]),1, 0)[:,:,1]
        ...
        y_coord, x_coord = int(com[0]), int(com[1]) # convert to integer coordinates for legibilty
...
## Requires RealSense for depth image and XY pixel position
def Locate3D(rs, x_pixel, y_pixel):
    ...
    h = rs.depth_np[y_pixel][x_pixel] # stay with mm

    ## Obtain angles of center pixels from sensor datum
    if x_pixel < (rs.width/2):
        rx = -(rs.width/2-x_pixel) / (rs.width/2) * WIDTH_FOV_ANGLE
    else:
        rx = ((x_pixel-rs.width/2) / (rs.width/2)) * WIDTH_FOV_ANGLE

    if y_pixel < (rs.height/2):
        ry = (rs.height/2-y_pixel) / (rs.height/2) * HEIGHT_FOV_ANGLE            
    else:
        ry = -((y_pixel-rs.height/2) / (rs.height/2)) * HEIGHT_FOV_ANGLE
...
def drawCenter(color_np, x_pixel, y_pixel, x, y, z, color=(0,255,0), target_class=None):
    ## Bail if no coordinates given
...
def drawText(color_np, x_pixel, y_pixel, x, y, z, color=(0,255,0), target_class=None):
    ...
    cv2.putText(color_np, '{} found'.format(target_class), (x_pixel+5,y_pixel+20), font, 0.75, color, 1, cv2.LINE_AA)
    cv2.putText(color_np, 'X: {}mm'.format(x),  (x_pixel+5,y_pixel+45), font, 0.75, color, 1, cv2.LINE_AA)
    cv2.putText(color_np, 'Y: {}mm'.format(y), (x_pixel+5,y_pixel+70), font, 0.75, color, 1, cv2.LINE_AA)
    cv2.putText(color_np, 'Z: {}mm'.format(z), (x_pixel+5,y_pixel+95), font, 0.75, color, 1, cv2.LINE_AA)
...
def drawBox(color_np, left,right,top,bottom, color=(0,255,0), target_class=None):
    ...
    # Annotate image
    cv2.rectangle(color_np, (left, top), (right, bottom), color, thickness) # annotate image
