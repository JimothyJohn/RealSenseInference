...
try:
    while fileIndex > 0:
        ...
        grey_color = 153
        maskImage = np.where(
            (depth_image > minDepth * 1.2) | (depth_image <= 0), 0, 255
        )

        # warm up time
        np.savetxt(
            "{}/3D-data/depth_{:05d}.csv".format(args.path, fileIndex),
            depth_image,
            delimiter=",",
        )
        cv2.imwrite(
            "{}/2D-data/img_{:05d}.png".format(args.path, fileIndex), color_image
        )
        cv2.imwrite("{}/labels/seg_{:05d}.png".format(args.path, fileIndex), maskImage)
        ...
        if WINDOW:
            ...
            bg_removed = np.where(
                (depth_image_3d > minDepth * 1.2) | (depth_image_3d <= 0),
                grey_color,
                color_image,
            )
